...
interface CookingSessionChatProps {
  isOpen: boolean;
  onClose: () => void;
  recipe: Recipe;
  currentStep: number;
...
export function CookingSessionChat({
  isOpen,
  onClose,
  recipe,
  currentStep,
}: CookingSessionChatProps) {
  ...
  // Contextual quick questions based on current step
  const quickQuestions = [
    `Help with step ${currentStep + 1}`,
      ...
      setMessages([
        {
          id: "1",
          role: "assistant",
          content: `Hi! I'm your cooking assistant for **${recipe.name}**. You're currently on **Step ${currentStep + 1}**: "${currentInstruction}". I can help with techniques, substitutions, timing, and any questions. What would you like to know?`,
          timestamp: new Date(),
        },
      ]);
    }
  }, [isOpen, recipe.name, currentStep, currentInstruction, messages.length]);

  // Update context message when step changes
  useEffect(() => {
    if (isOpen && messages.length > 0 && currentStep !== undefined) {
      const contextMessage: Message = {
        id: Date.now().toString(),
        role: "assistant",
        content: `You're now on **Step ${currentStep + 1}**: "${currentInstruction}". How can I help with this step?`,
        timestamp: new Date(),
      };
          ...
          >
            ...
            <div className="p-4 border-t-2 border-[#fcf45a]/30 bg-[#1d7b86]">
              ...
              <p className="text-xs text-white/50 mt-2 font-body">
                Press{" "}
                <kbd className="px-1.5 py-0.5 rounded bg-[#428a93] border border-white/20 font-mono">
                  Enter
                </kbd>{" "}
                to send,{" "}
                <kbd className="px-1.5 py-0.5 rounded bg-[#428a93] border border-white/20 font-mono">
